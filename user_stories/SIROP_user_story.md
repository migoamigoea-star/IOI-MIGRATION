# User Story — Stock Item Request Other Purpose (`SIROP`)
...
SIROP is migrated as a STR workflow form in MainDB_STR with FormCode=SIROP. The implementation
...
> I want to **create and submit form** in the `SIROP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SIROP

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SIROP` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SIROP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Stock Item Request Other Purpose records**
> As an **authorized user**,  
> I want to search, filter, and view Stock Item Request Other Purpose records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SIROP`                                       |
...
|--------|---------|------------|
| SIROP_List | Search and filter SIROP records | STR readers, reviewers, approvers |
| SIROP_New | Create new SIROP request | STR requestors |
| SIROP_View | Read-only detail view | All authorized users |
| SIROP_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SIROP_List Screen**
- Gallery displaying all `SIROP` records from `MainDB_STR`.
...
- `+ New` button navigates to `SIROP_New` (visible to Initiator role only).
- Tap a row to navigate to `SIROP_View`.

**SIROP_New / _Edit Screen**
...
Set(varTitle, "SIROP-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SIROP_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SIROP_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SIROP_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SIROP` (system-enforced constant).
...
| ------- | ----------------- | ----------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_SIROP_Submit  | Item created (FormCode=SIROP) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SIROP_Review  | CurrentAction=Review          | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SIROP_Approve | Reviewer decision=Approve     | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SIROP_Close   | Final state reached           | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SIROP_List`, `SIROP_New`, `SIROP_View`, `SIROP_Edit`) are functional.
