# User Story — STOCK ITEM CANCELLATION FORM (`SIC`)
...
SIC is migrated as a STR workflow form in MainDB_STR with FormCode=SIC. The implementation preserves
submission, review/approval routing, auditability, and notification behavior for PRAI operations.
...
> I want to **create and submit form** in the `SIC` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SIC

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SIC` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SIC` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter STOCK ITEM CANCELLATION FORM records**
> As an **authorized user**,  
> I want to search, filter, and view STOCK ITEM CANCELLATION FORM records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SIC`                                         |
...
|--------|---------|------------|
| SIC_List | Search and filter SIC records | STR readers, reviewers, approvers |
| SIC_New | Create new SIC request | STR requestors |
| SIC_View | Read-only detail view | All authorized users |
| SIC_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SIC_List Screen**
- Gallery displaying all `SIC` records from `MainDB_STR`.
...
- `+ New` button navigates to `SIC_New` (visible to Initiator role only).
- Tap a row to navigate to `SIC_View`.

**SIC_New / _Edit Screen**
...
Set(varTitle, "SIC-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SIC_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SIC_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SIC_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SIC` (system-enforced constant).
...
| ------- | --------------- | --------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_SIC_Submit  | Item created (FormCode=SIC) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SIC_Review  | CurrentAction=Review        | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SIC_Approve | Reviewer decision=Approve   | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SIC_Close   | Final state reached         | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SIC_List`, `SIC_New`, `SIC_View`, `SIC_Edit`) are functional.
