# User Story — Stock Item Cancellation Subform (`SIC_SF`)
...
SIC_SF is migrated as a STR workflow form in MainDB_STR with FormCode=SIC_SF. The implementation
...
> I want to **create and submit form** in the `SIC_SF` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SIC_SF

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SIC_SF` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SIC_SF` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Stock Item Cancellation Subform records**
> As an **authorized user**,  
> I want to search, filter, and view Stock Item Cancellation Subform records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SIC_SF`                                      |
...
|--------|---------|------------|
| SIC_SF_List | Search and filter SIC_SF records | STR readers, reviewers, approvers |
| SIC_SF_New | Create new SIC_SF request | STR requestors |
| SIC_SF_View | Read-only detail view | All authorized users |
| SIC_SF_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SIC_SF_List Screen**
- Gallery displaying all `SIC_SF` records from `MainDB_STR`.
...
- `+ New` button navigates to `SIC_SF_New` (visible to Initiator role only).
- Tap a row to navigate to `SIC_SF_View`.

**SIC_SF_New / _Edit Screen**
...
Set(varTitle, "SIC_SF-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SIC_SF_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SIC_SF_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SIC_SF_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SIC_SF` (system-enforced constant).
...
| ------- | ------------------ | ------------------------------ | ------------------------------------------------------------------------ |
| Submit  | STR_SIC_SF_Submit  | Item created (FormCode=SIC_SF) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SIC_SF_Review  | CurrentAction=Review           | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SIC_SF_Approve | Reviewer decision=Approve      | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SIC_SF_Close   | Final state reached            | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SIC_SF_List`, `SIC_SF_New`, `SIC_SF_View`, `SIC_SF_Edit`) are functional.
