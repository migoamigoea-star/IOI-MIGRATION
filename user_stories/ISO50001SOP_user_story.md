# User Story — ISO50001SOP (`ISO50001SOP`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `ISO50001SOP`

---

## 1. App Overview & Purpose

`ISO50001SOP` is migrated from Lotus Domino to Microsoft 365 (Power Apps + SharePoint Online). The app supports standardized submission, review, and approval workflows for the `` department at the  site.

---

## 2. User Stories

**US-LIST: Search and filter ISO50001SOP records**
> As an **authorized user**,  
> I want to search, filter, and view ISO50001SOP records in the list screen,  
...
|--------|---------|------------|
| `ISO50001SOP_List` | Browse and search all ISO50001SOP records | All authorized users |
| `ISO50001SOP_New` | Create a new ISO50001SOP request | Initiator / Requestor |
| `ISO50001SOP_View` | Read-only detail view of a record | All authorized users |
| `ISO50001SOP_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `ISO50001SOP_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**ISO50001SOP_List Screen**
- Gallery displaying all `ISO50001SOP` records from `MainDB_`.
...
- `+ New` button navigates to `ISO50001SOP_New` (visible to Initiator role only).
- Tap a row to navigate to `ISO50001SOP_View`.

**ISO50001SOP_New / _Edit Screen**
...
Set(varTitle, "ISO50001SOP-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ISO50001SOP_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ISO50001SOP_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "ISO50001SOP",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(ISO50001SOP_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ISO50001SOP` (system-enforced constant).
...
- [ ] Canvas App screens (`ISO50001SOP_List`, `ISO50001SOP_New`, `ISO50001SOP_View`, `ISO50001SOP_Edit`) are functional.
