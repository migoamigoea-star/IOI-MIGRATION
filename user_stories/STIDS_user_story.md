# User Story — Store Incoming Delivery (`STIDS`)
...
STIDS is migrated as a STR workflow form in MainDB_STR with FormCode=STIDS. The implementation
...
> I want to **create and submit form** in the `STIDS` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=STIDS

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `STIDS` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `STIDS` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Store Incoming Delivery records**
> As an **authorized user**,  
> I want to search, filter, and view Store Incoming Delivery records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `STIDS`                                       |
...
|--------|---------|------------|
| STIDS_List | Search and filter STIDS records | STR readers, reviewers, approvers |
| STIDS_New | Create new STIDS request | STR requestors |
| STIDS_View | Read-only detail view | All authorized users |
| STIDS_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**STIDS_List Screen**
- Gallery displaying all `STIDS` records from `MainDB_STR`.
...
- `+ New` button navigates to `STIDS_New` (visible to Initiator role only).
- Tap a row to navigate to `STIDS_View`.

**STIDS_New / _Edit Screen**
...
Set(varTitle, "STIDS-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(STIDS_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(STIDS_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(STIDS_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `STIDS` (system-enforced constant).
...
| ------- | ----------------- | ----------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_STIDS_Submit  | Item created (FormCode=STIDS) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_STIDS_Review  | CurrentAction=Review          | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_STIDS_Approve | Reviewer decision=Approve     | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_STIDS_Close   | Final state reached           | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`STIDS_List`, `STIDS_New`, `STIDS_View`, `STIDS_Edit`) are functional.
