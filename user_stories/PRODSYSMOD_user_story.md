# User Story — PRODSYSMOD (`PRODSYSMOD`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `PRODSYSMOD`

---

## 1. App Overview & Purpose

`PRODSYSMOD` is migrated from Lotus Domino to Microsoft 365 (Power Apps + SharePoint Online). The app supports standardized submission, review, and approval workflows for the `` department at the  site.

---

## 2. User Stories

**US-LIST: Search and filter PRODSYSMOD records**
> As an **authorized user**,  
> I want to search, filter, and view PRODSYSMOD records in the list screen,  
...
|--------|---------|------------|
| `PRODSYSMOD_List` | Browse and search all PRODSYSMOD records | All authorized users |
| `PRODSYSMOD_New` | Create a new PRODSYSMOD request | Initiator / Requestor |
| `PRODSYSMOD_View` | Read-only detail view of a record | All authorized users |
| `PRODSYSMOD_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `PRODSYSMOD_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**PRODSYSMOD_List Screen**
- Gallery displaying all `PRODSYSMOD` records from `MainDB_`.
...
- `+ New` button navigates to `PRODSYSMOD_New` (visible to Initiator role only).
- Tap a row to navigate to `PRODSYSMOD_View`.

**PRODSYSMOD_New / _Edit Screen**
...
Set(varTitle, "PRODSYSMOD-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(PRODSYSMOD_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(PRODSYSMOD_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(PRODSYSMOD_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `PRODSYSMOD` (system-enforced constant).
...
- [ ] Canvas App screens (`PRODSYSMOD_List`, `PRODSYSMOD_New`, `PRODSYSMOD_View`, `PRODSYSMOD_Edit`) are functional.
