...
`ECMCALI` is migrated from Lotus Domino to Microsoft 365 (Power Apps + SharePoint Online). The app supports standardized submission, review, and approval workflows for the `` department at the  site.

---

## 2. User Stories

**US-LIST: Search and filter ECMCALI records**
> As an **authorized user**,  
> I want to search, filter, and view ECMCALI records in the list screen,  
...
|--------|---------|------------|
| `ECMCALI_List` | Browse and search all ECMCALI records | All authorized users |
| `ECMCALI_New` | Create a new ECMCALI request | Initiator / Requestor |
| `ECMCALI_View` | Read-only detail view of a record | All authorized users |
| `ECMCALI_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `ECMCALI_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**ECMCALI_List Screen**
- Gallery displaying all `ECMCALI` records from `MainDB_`.
...
- `+ New` button navigates to `ECMCALI_New` (visible to Initiator role only).
- Tap a row to navigate to `ECMCALI_View`.

**ECMCALI_New / _Edit Screen**
...
Set(varTitle, "ECMCALI-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ECMCALI_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ECMCALI_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(ECMCALI_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ECMCALI` (system-enforced constant).
...
- [ ] Canvas App screens (`ECMCALI_List`, `ECMCALI_New`, `ECMCALI_View`, `ECMCALI_Edit`) are functional.
