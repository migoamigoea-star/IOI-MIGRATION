# User Story — PCN Stock Item (`PCSTKITEM`)
...
> I want to **create and submit form** in the `PCSTKITEM` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=PCSTKITEM

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `PCSTKITEM` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `PCSTKITEM` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter PCN Stock Item records**
> As an **authorized user**,  
> I want to search, filter, and view PCN Stock Item records in the list screen,  
...
**PCSTKITEM_List Screen**
- Gallery displaying all `PCSTKITEM` records from `MainDB_STR`.
...
- `+ New` button navigates to `PCSTKITEM_New` (visible to Initiator role only).
- Tap a row to navigate to `PCSTKITEM_View`.

**PCSTKITEM_New / _Edit Screen**
...
Set(varTitle, "PCSTKITEM-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(PCSTKITEM_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(PCSTKITEM_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(PCSTKITEM_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `PCSTKITEM` (system-enforced constant).
...
- [ ] Canvas App screens (`PCSTKITEM_List`, `PCSTKITEM_New`, `PCSTKITEM_View`, `PCSTKITEM_Edit`) are functional.
