# User Story — 3_SAPSLoc_Internal (`3_SAPSLoc_Internal`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `3_SAPSLoc_Internal`
...
**US-LIST: Search and filter 3_SAPSLoc_Internal records**
> As an **authorized user**,  
> I want to search, filter, and view 3_SAPSLoc_Internal records in the list screen,  
...
|--------|---------|------------|
| `3_SAPSLoc_Internal_List` | Browse and search all 3_SAPSLoc_Internal records | All authorized users |
| `3_SAPSLoc_Internal_New` | Create a new 3_SAPSLoc_Internal request | Initiator / Requestor |
| `3_SAPSLoc_Internal_View` | Read-only detail view of a record | All authorized users |
| `3_SAPSLoc_Internal_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `3_SAPSLoc_Internal_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**3_SAPSLoc_Internal_List Screen**
- Gallery displaying all `3_SAPSLoc_Internal` records from `MainDB_`.
...
- `+ New` button navigates to `3_SAPSLoc_Internal_New` (visible to Initiator role only).
- Tap a row to navigate to `3_SAPSLoc_Internal_View`.

**3_SAPSLoc_Internal_New / _Edit Screen**
...
**3_SAPSLoc_Internal_View Screen**
...
Set(varTitle, "3_SAPSLoc_Internal-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(3_SAPSLoc_Internal_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(3_SAPSLoc_Internal_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "3_SAPSLoc_Internal",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(3_SAPSLoc_Internal_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `3_SAPSLoc_Internal` (system-enforced constant).
...
- [ ] Canvas App screens (`3_SAPSLoc_Internal_List`, `3_SAPSLoc_Internal_New`, `3_SAPSLoc_Internal_View`, `3_SAPSLoc_Internal_Edit`) are functional.
