...
> I want to **create draft** in the `EPHR` application,  
...
> I want to **review** in the `EPHR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **final decision** in the `EPHR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review outcome submitted

**US-LIST: Search and filter EPHR Form records**
> As an **authorized user**,  
> I want to search, filter, and view EPHR Form records in the list screen,  
...
**EPHR_List Screen**
- Gallery displaying all `EPHR` records from `MainDB_HR`.
...
- `+ New` button navigates to `EPHR_New` (visible to Initiator role only).
- Tap a row to navigate to `EPHR_View`.

**EPHR_New / _Edit Screen**
...
Set(varTitle, "EPHR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(EPHR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(EPHR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(EPHR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `EPHR` (system-enforced constant).
...
- [ ] Canvas App screens (`EPHR_List`, `EPHR_New`, `EPHR_View`, `EPHR_Edit`) are functional.
