# User Story — ACM STOCK ITEM REQUEST FOR OTHER (`ACMSTR`)
...
ACMSTR is migrated as a STR workflow form in MainDB_STR with FormCode=ACMSTR. The implementation
...
> I want to **create and submit form** in the `ACMSTR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=ACMSTR

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `ACMSTR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `ACMSTR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter ACM STOCK ITEM REQUEST FOR OTHER records**
> As an **authorized user**,  
> I want to search, filter, and view ACM STOCK ITEM REQUEST FOR OTHER records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `ACMSTR`                                      |
...
|--------|---------|------------|
| ACMSTR_List | Search and filter ACMSTR records | STR readers, reviewers, approvers |
| ACMSTR_New | Create new ACMSTR request | STR requestors |
| ACMSTR_View | Read-only detail view | All authorized users |
| ACMSTR_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**ACMSTR_List Screen**
- Gallery displaying all `ACMSTR` records from `MainDB_STR`.
...
- `+ New` button navigates to `ACMSTR_New` (visible to Initiator role only).
- Tap a row to navigate to `ACMSTR_View`.

**ACMSTR_New / _Edit Screen**
...
Set(varTitle, "ACMSTR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ACMSTR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ACMSTR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(ACMSTR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ACMSTR` (system-enforced constant).
...
| ------- | ------------------ | ------------------------------ | ------------------------------------------------------------------------ |
| Submit  | STR_ACMSTR_Submit  | Item created (FormCode=ACMSTR) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_ACMSTR_Review  | CurrentAction=Review           | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_ACMSTR_Approve | Reviewer decision=Approve      | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_ACMSTR_Close   | Final state reached            | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`ACMSTR_List`, `ACMSTR_New`, `ACMSTR_View`, `ACMSTR_Edit`) are functional.
