...
IT Event is migrated as an IT workflow form in MainDB_IT with FormCode=IAL3. The implementation
...
> I want to **create and submit form** in the `IAL3` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=IAL3

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `IAL3` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `IAL3` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter IT Event records**
> As an **authorized user**,  
> I want to search, filter, and view IT Event records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `IAL3`                                        |
...
|--------|---------|------------|
| IAL3_List | Search and filter IAL3 records | IT readers, reviewers, approvers |
| IAL3_New | Create new IAL3 request | IT requestors |
| IAL3_View | Read-only detail view | All authorized users |
| IAL3_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**IAL3_List Screen**
- Gallery displaying all `IAL3` records from `MainDB_IT`.
...
- `+ New` button navigates to `IAL3_New` (visible to Initiator role only).
- Tap a row to navigate to `IAL3_View`.

**IAL3_New / _Edit Screen**
...
Set(varTitle, "IAL3-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(IAL3_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(IAL3_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(IAL3_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `IAL3` (system-enforced constant).
...
| ------- | --------------- | ---------------------------- | ------------------------------------------------------------------------ |
| Submit  | IT_IAL3_Submit  | Item created (FormCode=IAL3) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | IT_IAL3_Review  | CurrentAction=Review         | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | IT_IAL3_Approve | Reviewer decision=Approve    | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | IT_IAL3_Close   | Final state reached          | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`IAL3_List`, `IAL3_New`, `IAL3_View`, `IAL3_Edit`) are functional.
