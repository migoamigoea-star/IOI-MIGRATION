...
This blueprint defines the migration baseline for Dbpay from Johor site source forms into
...
> I want to **draft entry** in the `DBPAY` application,  
...
**US-02: Submit**
> As a **Initiator** (member of `SA-Initiators`),  
> I want to **submit** in the `DBPAY` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **review** in the `DBPAY` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **final decision** in the `DBPAY` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* Decision action updates final status

**US-LIST: Search and filter Dbpay records**
> As an **authorized user**,  
> I want to search, filter, and view Dbpay records in the list screen,  
...
| 2   | FormCode       | Single line of text | Yes      | Form code           | Fixed value DBPAY                            |
...
|--------|---------|------------|
| DBPAY_List | Record listing and search | All authenticated roles |
| DBPAY_New | New submission form | Initiators |
| DBPAY_View | Read-only details | All authenticated roles |
| DBPAY_Edit | Editable review form | Reviewers and approvers |

### Screen Interaction Details

**DBPAY_List Screen**
- Gallery displaying all `DBPAY` records from `MainDB_SA`.
...
- `+ New` button navigates to `DBPAY_New` (visible to Initiator role only).
- Tap a row to navigate to `DBPAY_View`.

**DBPAY_New / _Edit Screen**
...
Set(varTitle, "DBPAY-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(DBPAY_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(DBPAY_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(DBPAY_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `DBPAY` (system-enforced constant).
...
| ------------------------ | ------------------------ | ------------------------------------------------------------ |
| SA_DBPAY_OnSubmit        | SharePoint item created  | Normalize fields, set FormCode, set CDate, notify reviewer   |
| SA_DBPAY_OnReview        | SharePoint item modified | Track review decision, append audit fields, notify approver  |
| SA_DBPAY_OnFinalDecision | SharePoint item modified | Apply final status and lock rules, send closure notification |
...
- [ ] Canvas App screens (`DBPAY_List`, `DBPAY_New`, `DBPAY_View`, `DBPAY_Edit`) are functional.
