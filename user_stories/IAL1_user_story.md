# User Story — Server Reboot Log (`IAL1`)
...
Server Reboot Log is migrated as an IT workflow form in MainDB_IT with FormCode=IAL1. The
...
> I want to **create and submit form** in the `IAL1` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=IAL1

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `IAL1` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `IAL1` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Server Reboot Log records**
> As an **authorized user**,  
> I want to search, filter, and view Server Reboot Log records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `IAL1`                                        |
...
|--------|---------|------------|
| IAL1_List | Search and filter IAL1 records | IT readers, reviewers, approvers |
| IAL1_New | Create new IAL1 request | IT requestors |
| IAL1_View | Read-only detail view | All authorized users |
| IAL1_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**IAL1_List Screen**
- Gallery displaying all `IAL1` records from `MainDB_IT`.
...
- `+ New` button navigates to `IAL1_New` (visible to Initiator role only).
- Tap a row to navigate to `IAL1_View`.

**IAL1_New / _Edit Screen**
...
Set(varTitle, "IAL1-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(IAL1_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(IAL1_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(IAL1_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `IAL1` (system-enforced constant).
...
| ------- | --------------- | ---------------------------- | ------------------------------------------------------------------------ |
| Submit  | IT_IAL1_Submit  | Item created (FormCode=IAL1) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | IT_IAL1_Review  | CurrentAction=Review         | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | IT_IAL1_Approve | Reviewer decision=Approve    | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | IT_IAL1_Close   | Final state reached          | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`IAL1_List`, `IAL1_New`, `IAL1_View`, `IAL1_Edit`) are functional.
