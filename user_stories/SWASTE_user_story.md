...
> I want to **create and submit form** in the `SWASTE` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SWASTE

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SWASTE` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SWASTE` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Store Waste records**
> As an **authorized user**,  
> I want to search, filter, and view Store Waste records in the list screen,  
...
**SWASTE_List Screen**
- Gallery displaying all `SWASTE` records from `MainDB_STR`.
...
- `+ New` button navigates to `SWASTE_New` (visible to Initiator role only).
- Tap a row to navigate to `SWASTE_View`.

**SWASTE_New / _Edit Screen**
...
Set(varTitle, "SWASTE-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SWASTE_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SWASTE_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SWASTE_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SWASTE` (system-enforced constant).
...
- [ ] Canvas App screens (`SWASTE_List`, `SWASTE_New`, `SWASTE_View`, `SWASTE_Edit`) are functional.
