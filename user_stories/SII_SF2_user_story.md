# User Story — Head of Dept's Approval - PROCESS OWNER (`SII_SF2`)
...
SII_SF2 is migrated as a STR workflow form in MainDB_STR with FormCode=SII_SF2. The implementation
...
> I want to **create and submit form** in the `SII_SF2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SII_SF2

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SII_SF2` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SII_SF2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Head of Dept's Approval - PROCESS OWNER records**
> As an **authorized user**,  
> I want to search, filter, and view Head of Dept's Approval - PROCESS OWNER records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SII_SF2`                                     |
...
|--------|---------|------------|
| SII_SF2_List | Search and filter SII_SF2 records | STR readers, reviewers, approvers |
| SII_SF2_New | Create new SII_SF2 request | STR requestors |
| SII_SF2_View | Read-only detail view | All authorized users |
| SII_SF2_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SII_SF2_List Screen**
- Gallery displaying all `SII_SF2` records from `MainDB_STR`.
...
- `+ New` button navigates to `SII_SF2_New` (visible to Initiator role only).
- Tap a row to navigate to `SII_SF2_View`.

**SII_SF2_New / _Edit Screen**
...
Set(varTitle, "SII_SF2-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SII_SF2_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SII_SF2_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SII_SF2_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SII_SF2` (system-enforced constant).
...
| ------- | ------------------- | ------------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_SII_SF2_Submit  | Item created (FormCode=SII_SF2) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SII_SF2_Review  | CurrentAction=Review            | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SII_SF2_Approve | Reviewer decision=Approve       | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SII_SF2_Close   | Final state reached             | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SII_SF2_List`, `SII_SF2_New`, `SII_SF2_View`, `SII_SF2_Edit`) are functional.
