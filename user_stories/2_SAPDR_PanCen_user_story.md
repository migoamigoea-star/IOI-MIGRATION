# User Story — 2_SAPDR_PanCen (`2_SAPDR_PanCen`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `2_SAPDR_PanCen`
...
**US-LIST: Search and filter 2_SAPDR_PanCen records**
> As an **authorized user**,  
> I want to search, filter, and view 2_SAPDR_PanCen records in the list screen,  
...
|--------|---------|------------|
| `2_SAPDR_PanCen_List` | Browse and search all 2_SAPDR_PanCen records | All authorized users |
| `2_SAPDR_PanCen_New` | Create a new 2_SAPDR_PanCen request | Initiator / Requestor |
| `2_SAPDR_PanCen_View` | Read-only detail view of a record | All authorized users |
| `2_SAPDR_PanCen_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `2_SAPDR_PanCen_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**2_SAPDR_PanCen_List Screen**
- Gallery displaying all `2_SAPDR_PanCen` records from `MainDB_`.
...
- `+ New` button navigates to `2_SAPDR_PanCen_New` (visible to Initiator role only).
- Tap a row to navigate to `2_SAPDR_PanCen_View`.

**2_SAPDR_PanCen_New / _Edit Screen**
...
Set(varTitle, "2_SAPDR_PanCen-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(2_SAPDR_PanCen_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(2_SAPDR_PanCen_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "2_SAPDR_PanCen",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(2_SAPDR_PanCen_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `2_SAPDR_PanCen` (system-enforced constant).
...
- [ ] Canvas App screens (`2_SAPDR_PanCen_List`, `2_SAPDR_PanCen_New`, `2_SAPDR_PanCen_View`, `2_SAPDR_PanCen_Edit`) are functional.
