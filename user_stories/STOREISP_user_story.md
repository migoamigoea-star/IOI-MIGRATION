# User Story — Store Incoming Shipping (`STOREISP`)

> **Department:** STR  
> **Module:** M3 - Store Operations  
> **Site(s):** PRAI  
...
STOREISP is migrated as a STR workflow form in MainDB_STR with FormCode=STOREISP. The implementation
...
> I want to **create and submit form** in the `STOREISP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=STOREISP

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `STOREISP` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `STOREISP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Store Incoming Shipping records**
> As an **authorized user**,  
> I want to search, filter, and view Store Incoming Shipping records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `STOREISP`                                    |
...
|--------|---------|------------|
| STOREISP_List | Search and filter STOREISP records | STR readers, reviewers, approvers |
| STOREISP_New | Create new STOREISP request | STR requestors |
| STOREISP_View | Read-only detail view | All authorized users |
| STOREISP_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**STOREISP_List Screen**
- Gallery displaying all `STOREISP` records from `MainDB_STR`.
...
- `+ New` button navigates to `STOREISP_New` (visible to Initiator role only).
- Tap a row to navigate to `STOREISP_View`.

**STOREISP_New / _Edit Screen**
...
Set(varTitle, "STOREISP-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(STOREISP_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(STOREISP_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(STOREISP_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `STOREISP` (system-enforced constant).
...
| ------- | -------------------- | -------------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_STOREISP_Submit  | Item created (FormCode=STOREISP) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_STOREISP_Review  | CurrentAction=Review             | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_STOREISP_Approve | Reviewer decision=Approve        | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_STOREISP_Close   | Final state reached              | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`STOREISP_List`, `STOREISP_New`, `STOREISP_View`, `STOREISP_Edit`) are functional.
