# User Story — SAP Disaster Recovery (DRP) (`SAPDRP`)
...
SAP Disaster Recovery (DRP) is migrated as an IT workflow form in MainDB_IT with FormCode=SAPDRP.
...
> I want to **create and submit form** in the `SAPDRP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SAPDRP

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SAPDRP` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SAPDRP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter SAP Disaster Recovery (DRP) records**
> As an **authorized user**,  
> I want to search, filter, and view SAP Disaster Recovery (DRP) records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SAPDRP`                                      |
...
|--------|---------|------------|
| SAPDRP_List | Search and filter SAPDRP records | IT readers, reviewers, approvers |
| SAPDRP_New | Create new SAPDRP request | IT requestors |
| SAPDRP_View | Read-only detail view | All authorized users |
| SAPDRP_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SAPDRP_List Screen**
- Gallery displaying all `SAPDRP` records from `MainDB_IT`.
...
- `+ New` button navigates to `SAPDRP_New` (visible to Initiator role only).
- Tap a row to navigate to `SAPDRP_View`.

**SAPDRP_New / _Edit Screen**
...
Set(varTitle, "SAPDRP-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SAPDRP_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SAPDRP_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SAPDRP_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SAPDRP` (system-enforced constant).
...
| ------- | ----------------- | ------------------------------ | ------------------------------------------------------------------------ |
| Submit  | IT_SAPDRP_Submit  | Item created (FormCode=SAPDRP) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | IT_SAPDRP_Review  | CurrentAction=Review           | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | IT_SAPDRP_Approve | Reviewer decision=Approve      | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | IT_SAPDRP_Close   | Final state reached            | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SAPDRP_List`, `SAPDRP_New`, `SAPDRP_View`, `SAPDRP_Edit`) are functional.
