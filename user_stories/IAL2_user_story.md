# User Story — Data Center Access Log (`IAL2`)
...
> I want to **create and submit form** in the `IAL2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=IAL2

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `IAL2` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `IAL2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Data Center Access Log records**
> As an **authorized user**,  
> I want to search, filter, and view Data Center Access Log records in the list screen,  
...
**IAL2_List Screen**
- Gallery displaying all `IAL2` records from `MainDB_IT`.
...
- `+ New` button navigates to `IAL2_New` (visible to Initiator role only).
- Tap a row to navigate to `IAL2_View`.

**IAL2_New / _Edit Screen**
...
Set(varTitle, "IAL2-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(IAL2_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(IAL2_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(IAL2_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `IAL2` (system-enforced constant).
...
- [ ] Canvas App screens (`IAL2_List`, `IAL2_New`, `IAL2_View`, `IAL2_Edit`) are functional.
