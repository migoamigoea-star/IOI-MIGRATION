# User Story — ISO50001NCR (`ISO50001NCR`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `ISO50001NCR`

---

## 1. App Overview & Purpose

`ISO50001NCR` is migrated from Lotus Domino to Microsoft 365 (Power Apps + SharePoint Online). The app supports standardized submission, review, and approval workflows for the `` department at the  site.

---

## 2. User Stories

**US-LIST: Search and filter ISO50001NCR records**
> As an **authorized user**,  
> I want to search, filter, and view ISO50001NCR records in the list screen,  
...
|--------|---------|------------|
| `ISO50001NCR_List` | Browse and search all ISO50001NCR records | All authorized users |
| `ISO50001NCR_New` | Create a new ISO50001NCR request | Initiator / Requestor |
| `ISO50001NCR_View` | Read-only detail view of a record | All authorized users |
| `ISO50001NCR_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `ISO50001NCR_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**ISO50001NCR_List Screen**
- Gallery displaying all `ISO50001NCR` records from `MainDB_`.
...
- `+ New` button navigates to `ISO50001NCR_New` (visible to Initiator role only).
- Tap a row to navigate to `ISO50001NCR_View`.

**ISO50001NCR_New / _Edit Screen**
...
Set(varTitle, "ISO50001NCR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ISO50001NCR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ISO50001NCR_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "ISO50001NCR",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(ISO50001NCR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ISO50001NCR` (system-enforced constant).
...
- [ ] Canvas App screens (`ISO50001NCR_List`, `ISO50001NCR_New`, `ISO50001NCR_View`, `ISO50001NCR_Edit`) are functional.
