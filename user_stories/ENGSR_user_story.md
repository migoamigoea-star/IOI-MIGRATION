...
**US-LIST: Search and filter ENGSR records**
> As an **authorized user**,  
> I want to search, filter, and view ENGSR records in the list screen,  
...
|--------|---------|------------|
| `ENGSR_List` | Browse and search all ENGSR records | All authorized users |
| `ENGSR_New` | Create a new ENGSR request | Initiator / Requestor |
| `ENGSR_View` | Read-only detail view of a record | All authorized users |
| `ENGSR_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `ENGSR_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**ENGSR_List Screen**
- Gallery displaying all `ENGSR` records from `MainDB_`.
...
- `+ New` button navigates to `ENGSR_New` (visible to Initiator role only).
- Tap a row to navigate to `ENGSR_View`.

**ENGSR_New / _Edit Screen**
...
Set(varTitle, "ENGSR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ENGSR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ENGSR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(ENGSR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ENGSR` (system-enforced constant).
...
- [ ] Canvas App screens (`ENGSR_List`, `ENGSR_New`, `ENGSR_View`, `ENGSR_Edit`) are functional.
