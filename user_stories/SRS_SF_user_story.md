...
> I want to **submit srs_sf** in the `SRS_SF` application,  
...
> I want to **review srs_sf** in the `SRS_SF` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **decide srs_sf** in the `SRS_SF` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When CurrentStatus = 'Approved' OR 'Rejected'

**US-LIST: Search and filter SRS_SF records**
> As an **authorized user**,  
> I want to search, filter, and view SRS_SF records in the list screen,  
...
**SRS_SF_List Screen**
- Gallery displaying all `SRS_SF` records from `MainDB_ADM`.
...
- `+ New` button navigates to `SRS_SF_New` (visible to Initiator role only).
- Tap a row to navigate to `SRS_SF_View`.

**SRS_SF_New / _Edit Screen**
...
Set(varTitle, "SRS_SF-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SRS_SF_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SRS_SF_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SRS_SF_List, ScreenTransition.Back)
```
...
- **Stage 1:** `Submit SRS_SF` — performed by `Initiator`
- **Stage 2:** `Review SRS_SF` — performed by `Reviewer`
- **Stage 3:** `Decide SRS_SF` — performed by `Approver`
...
- `FormCode` must always equal `SRS_SF` (system-enforced constant).
...
- [ ] Canvas App screens (`SRS_SF_List`, `SRS_SF_New`, `SRS_SF_View`, `SRS_SF_Edit`) are functional.
