# User Story — Cost Monitoring Form - Part A (`CMF_A`)
...
**US-01: Create & submit CMF_A**
> As a **Item created (FormCode=CMF_A)** (member of `Cost Centre Manager`),  
> I want to **create & submit cmf_a** in the `CMF_A` application,  
...
> I want to **finance review** in the `CMF_A` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **archive** in the `CMF_A` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* `D04-FIN-Admins`

**US-LIST: Search and filter Cost Monitoring Form - Part A records**
> As an **authorized user**,  
> I want to search, filter, and view Cost Monitoring Form - Part A records in the list screen,  
...
**CMF_A_List Screen**
- Gallery displaying all `CMF_A` records from `MainDB_FIN`.
...
- `+ New` button navigates to `CMF_A_New` (visible to Initiator role only).
- Tap a row to navigate to `CMF_A_View`.

**CMF_A_New / _Edit Screen**
...
Set(varTitle, "CMF_A-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(CMF_A_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(CMF_A_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(CMF_A_List, ScreenTransition.Back)
```
...
- **Stage 1:** `Create & submit CMF_A` — performed by `Item created (FormCode=CMF_A)`
- **Stage 2:** `Finance review` — performed by `Status=Submitted`
- **Stage 3:** `Archive` — performed by `Status=Approved`
...
- `FormCode` must always equal `CMF_A` (system-enforced constant).
...
- [ ] Canvas App screens (`CMF_A_List`, `CMF_A_New`, `CMF_A_View`, `CMF_A_Edit`) are functional.
