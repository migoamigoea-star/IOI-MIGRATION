# User Story — Returnable Items (`RETURNABLESITEMS`)

> **Department:** STR  
> **Module:** M4 - Returnable & Direct Items  
> **Site(s):** PRAI  
> **SharePoint List:** `MainDB_STR`  
> **Form Code:** `RETURNABLESITEMS`

---

## 1. App Overview & Purpose

RETURNABLESITEMS is migrated as a STR workflow form in MainDB_STR with FormCode=RETURNABLESITEMS.
The implementation preserves submission, review/approval routing, auditability, and notification
...
> I want to **create and submit form** in the `RETURNABLESITEMS` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=RETURNABLESITEMS

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `RETURNABLESITEMS` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `RETURNABLESITEMS` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Returnable Items records**
> As an **authorized user**,  
> I want to search, filter, and view Returnable Items records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `RETURNABLESITEMS`                            |
...
|--------|---------|------------|
| RETURNABLESITEMS_List | Search and filter RETURNABLESITEMS records | STR readers, reviewers, approvers |
| RETURNABLESITEMS_New | Create new RETURNABLESITEMS request | STR requestors |
| RETURNABLESITEMS_View | Read-only detail view | All authorized users |
| RETURNABLESITEMS_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**RETURNABLESITEMS_List Screen**
- Gallery displaying all `RETURNABLESITEMS` records from `MainDB_STR`.
...
- `+ New` button navigates to `RETURNABLESITEMS_New` (visible to Initiator role only).
- Tap a row to navigate to `RETURNABLESITEMS_View`.

**RETURNABLESITEMS_New / _Edit Screen**
...
Set(varTitle, "RETURNABLESITEMS-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(RETURNABLESITEMS_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(RETURNABLESITEMS_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "RETURNABLESITEMS",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(RETURNABLESITEMS_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `RETURNABLESITEMS` (system-enforced constant).
...
| ------- | ---------------------------- | ---------------------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_RETURNABLESITEMS_Submit  | Item created (FormCode=RETURNABLESITEMS) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_RETURNABLESITEMS_Review  | CurrentAction=Review                     | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_RETURNABLESITEMS_Approve | Reviewer decision=Approve                | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_RETURNABLESITEMS_Close   | Final state reached                      | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`RETURNABLESITEMS_List`, `RETURNABLESITEMS_New`, `RETURNABLESITEMS_View`, `RETURNABLESITEMS_Edit`) are functional.
