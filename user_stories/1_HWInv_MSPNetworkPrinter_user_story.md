# User Story — 1_HWInv_MSPNetworkPrinter (`1_HWInv_MSPNetworkPrinter`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `1_HWInv_MSPNetworkPrinter`
...
**US-LIST: Search and filter 1_HWInv_MSPNetworkPrinter records**
> As an **authorized user**,  
> I want to search, filter, and view 1_HWInv_MSPNetworkPrinter records in the list screen,  
...
|--------|---------|------------|
| `1_HWInv_MSPNetworkPrinter_List` | Browse and search all 1_HWInv_MSPNetworkPrinter records | All authorized users |
| `1_HWInv_MSPNetworkPrinter_New` | Create a new 1_HWInv_MSPNetworkPrinter request | Initiator / Requestor |
| `1_HWInv_MSPNetworkPrinter_View` | Read-only detail view of a record | All authorized users |
| `1_HWInv_MSPNetworkPrinter_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `1_HWInv_MSPNetworkPrinter_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**1_HWInv_MSPNetworkPrinter_List Screen**
- Gallery displaying all `1_HWInv_MSPNetworkPrinter` records from `MainDB_`.
...
- `+ New` button navigates to `1_HWInv_MSPNetworkPrinter_New` (visible to Initiator role only).
- Tap a row to navigate to `1_HWInv_MSPNetworkPrinter_View`.

**1_HWInv_MSPNetworkPrinter_New / _Edit Screen**
...
**1_HWInv_MSPNetworkPrinter_View Screen**
...
Set(varTitle, "1_HWInv_MSPNetworkPrinter-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(1_HWInv_MSPNetworkPrinter_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(1_HWInv_MSPNetworkPrinter_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "1_HWInv_MSPNetworkPrinter",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(1_HWInv_MSPNetworkPrinter_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `1_HWInv_MSPNetworkPrinter` (system-enforced constant).
...
- [ ] Canvas App screens (`1_HWInv_MSPNetworkPrinter_List`, `1_HWInv_MSPNetworkPrinter_New`, `1_HWInv_MSPNetworkPrinter_View`, `1_HWInv_MSPNetworkPrinter_Edit`) are functional.
