# User Story — Outstanding Tasks register (`ITDR2`)
...
Outstanding Tasks register is migrated as an IT workflow form in MainDB_IT with FormCode=ITDR2. The
...
> I want to **create and submit form** in the `ITDR2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=ITDR2

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `ITDR2` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `ITDR2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Outstanding Tasks register records**
> As an **authorized user**,  
> I want to search, filter, and view Outstanding Tasks register records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `ITDR2`                                       |
...
|--------|---------|------------|
| ITDR2_List | Search and filter ITDR2 records | IT readers, reviewers, approvers |
| ITDR2_New | Create new ITDR2 request | IT requestors |
| ITDR2_View | Read-only detail view | All authorized users |
| ITDR2_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**ITDR2_List Screen**
- Gallery displaying all `ITDR2` records from `MainDB_IT`.
...
- `+ New` button navigates to `ITDR2_New` (visible to Initiator role only).
- Tap a row to navigate to `ITDR2_View`.

**ITDR2_New / _Edit Screen**
...
Set(varTitle, "ITDR2-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ITDR2_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ITDR2_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(ITDR2_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ITDR2` (system-enforced constant).
...
| ------- | ---------------- | ----------------------------- | ------------------------------------------------------------------------ |
| Submit  | IT_ITDR2_Submit  | Item created (FormCode=ITDR2) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | IT_ITDR2_Review  | CurrentAction=Review          | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | IT_ITDR2_Approve | Reviewer decision=Approve     | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | IT_ITDR2_Close   | Final state reached           | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`ITDR2_List`, `ITDR2_New`, `ITDR2_View`, `ITDR2_Edit`) are functional.
