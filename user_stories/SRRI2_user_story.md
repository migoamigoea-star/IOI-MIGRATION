# User Story — Stock Return/Request Instruction 2 (`SRRI2`)
...
SRRI2 is migrated as a STR workflow form in MainDB_STR with FormCode=SRRI2. The implementation
...
> I want to **create and submit form** in the `SRRI2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SRRI2

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SRRI2` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SRRI2` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Stock Return/Request Instruction 2 records**
> As an **authorized user**,  
> I want to search, filter, and view Stock Return/Request Instruction 2 records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SRRI2`                                       |
...
|--------|---------|------------|
| SRRI2_List | Search and filter SRRI2 records | STR readers, reviewers, approvers |
| SRRI2_New | Create new SRRI2 request | STR requestors |
| SRRI2_View | Read-only detail view | All authorized users |
| SRRI2_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SRRI2_List Screen**
- Gallery displaying all `SRRI2` records from `MainDB_STR`.
...
- `+ New` button navigates to `SRRI2_New` (visible to Initiator role only).
- Tap a row to navigate to `SRRI2_View`.

**SRRI2_New / _Edit Screen**
...
Set(varTitle, "SRRI2-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SRRI2_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SRRI2_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SRRI2_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SRRI2` (system-enforced constant).
...
| ------- | ----------------- | ----------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_SRRI2_Submit  | Item created (FormCode=SRRI2) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SRRI2_Review  | CurrentAction=Review          | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SRRI2_Approve | Reviewer decision=Approve     | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SRRI2_Close   | Final state reached           | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SRRI2_List`, `SRRI2_New`, `SRRI2_View`, `SRRI2_Edit`) are functional.
