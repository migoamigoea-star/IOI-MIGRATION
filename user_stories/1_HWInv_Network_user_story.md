# User Story — 1_HWInv_Network (`1_HWInv_Network`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `1_HWInv_Network`
...
**US-LIST: Search and filter 1_HWInv_Network records**
> As an **authorized user**,  
> I want to search, filter, and view 1_HWInv_Network records in the list screen,  
...
|--------|---------|------------|
| `1_HWInv_Network_List` | Browse and search all 1_HWInv_Network records | All authorized users |
| `1_HWInv_Network_New` | Create a new 1_HWInv_Network request | Initiator / Requestor |
| `1_HWInv_Network_View` | Read-only detail view of a record | All authorized users |
| `1_HWInv_Network_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `1_HWInv_Network_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**1_HWInv_Network_List Screen**
- Gallery displaying all `1_HWInv_Network` records from `MainDB_`.
...
- `+ New` button navigates to `1_HWInv_Network_New` (visible to Initiator role only).
- Tap a row to navigate to `1_HWInv_Network_View`.

**1_HWInv_Network_New / _Edit Screen**
...
Set(varTitle, "1_HWInv_Network-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(1_HWInv_Network_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(1_HWInv_Network_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "1_HWInv_Network",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(1_HWInv_Network_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `1_HWInv_Network` (system-enforced constant).
...
- [ ] Canvas App screens (`1_HWInv_Network_List`, `1_HWInv_Network_New`, `1_HWInv_Network_View`, `1_HWInv_Network_Edit`) are functional.
