# User Story — SAP & WB Change Request Form (`SAPCR`)
...
SAP & WB Change Request Form is migrated as an IT workflow form in MainDB_IT with FormCode=SAPCR.
...
> I want to **create and submit form** in the `SAPCR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SAPCR

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SAPCR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SAPCR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter SAP & WB Change Request Form records**
> As an **authorized user**,  
> I want to search, filter, and view SAP & WB Change Request Form records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SAPCR`                                       |
...
|--------|---------|------------|
| SAPCR_List | Search and filter SAPCR records | IT readers, reviewers, approvers |
| SAPCR_New | Create new SAPCR request | IT requestors |
| SAPCR_View | Read-only detail view | All authorized users |
| SAPCR_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SAPCR_List Screen**
- Gallery displaying all `SAPCR` records from `MainDB_IT`.
...
- `+ New` button navigates to `SAPCR_New` (visible to Initiator role only).
- Tap a row to navigate to `SAPCR_View`.

**SAPCR_New / _Edit Screen**
...
Set(varTitle, "SAPCR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SAPCR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SAPCR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SAPCR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SAPCR` (system-enforced constant).
...
| ------- | ---------------- | ----------------------------- | ------------------------------------------------------------------------ |
| Submit  | IT_SAPCR_Submit  | Item created (FormCode=SAPCR) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | IT_SAPCR_Review  | CurrentAction=Review          | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | IT_SAPCR_Approve | Reviewer decision=Approve     | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | IT_SAPCR_Close   | Final state reached           | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SAPCR_List`, `SAPCR_New`, `SAPCR_View`, `SAPCR_Edit`) are functional.
