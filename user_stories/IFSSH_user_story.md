# User Story — Issuance Form For Safety (`IFSSH`)
...
IFSSH is migrated as a STR workflow form in MainDB_STR with FormCode=IFSSH. The implementation
...
> I want to **create and submit form** in the `IFSSH` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=IFSSH

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `IFSSH` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `IFSSH` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Issuance Form For Safety records**
> As an **authorized user**,  
> I want to search, filter, and view Issuance Form For Safety records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `IFSSH`                                       |
...
|--------|---------|------------|
| IFSSH_List | Search and filter IFSSH records | STR readers, reviewers, approvers |
| IFSSH_New | Create new IFSSH request | STR requestors |
| IFSSH_View | Read-only detail view | All authorized users |
| IFSSH_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**IFSSH_List Screen**
- Gallery displaying all `IFSSH` records from `MainDB_STR`.
...
- `+ New` button navigates to `IFSSH_New` (visible to Initiator role only).
- Tap a row to navigate to `IFSSH_View`.

**IFSSH_New / _Edit Screen**
...
Set(varTitle, "IFSSH-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(IFSSH_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(IFSSH_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(IFSSH_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `IFSSH` (system-enforced constant).
...
| ------- | ----------------- | ----------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_IFSSH_Submit  | Item created (FormCode=IFSSH) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_IFSSH_Review  | CurrentAction=Review          | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_IFSSH_Approve | Reviewer decision=Approve     | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_IFSSH_Close   | Final state reached           | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`IFSSH_List`, `IFSSH_New`, `IFSSH_View`, `IFSSH_Edit`) are functional.
