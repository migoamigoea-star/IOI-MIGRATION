# User Story — Material Return Form Application (`MRFAPP`)
...
MRFAPP is migrated as a STR workflow form in MainDB_STR with FormCode=MRFAPP. The implementation
...
> I want to **create and submit form** in the `MRFAPP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=MRFAPP

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `MRFAPP` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `MRFAPP` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Material Return Form Application records**
> As an **authorized user**,  
> I want to search, filter, and view Material Return Form Application records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `MRFAPP`                                      |
...
|--------|---------|------------|
| MRFAPP_List | Search and filter MRFAPP records | STR readers, reviewers, approvers |
| MRFAPP_New | Create new MRFAPP request | STR requestors |
| MRFAPP_View | Read-only detail view | All authorized users |
| MRFAPP_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**MRFAPP_List Screen**
- Gallery displaying all `MRFAPP` records from `MainDB_STR`.
...
- `+ New` button navigates to `MRFAPP_New` (visible to Initiator role only).
- Tap a row to navigate to `MRFAPP_View`.

**MRFAPP_New / _Edit Screen**
...
Set(varTitle, "MRFAPP-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(MRFAPP_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(MRFAPP_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(MRFAPP_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `MRFAPP` (system-enforced constant).
...
| ------- | ------------------ | ------------------------------ | ------------------------------------------------------------------------ |
| Submit  | STR_MRFAPP_Submit  | Item created (FormCode=MRFAPP) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_MRFAPP_Review  | CurrentAction=Review           | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_MRFAPP_Approve | Reviewer decision=Approve      | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_MRFAPP_Close   | Final state reached            | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`MRFAPP_List`, `MRFAPP_New`, `MRFAPP_View`, `MRFAPP_Edit`) are functional.
