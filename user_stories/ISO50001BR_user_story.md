# User Story — ISO50001BR (`ISO50001BR`)

> **Department:**   
> **Module:**   
> **Site(s):**   
> **SharePoint List:** `MainDB_`  
> **Form Code:** `ISO50001BR`

---

## 1. App Overview & Purpose

`ISO50001BR` is migrated from Lotus Domino to Microsoft 365 (Power Apps + SharePoint Online). The app supports standardized submission, review, and approval workflows for the `` department at the  site.

---

## 2. User Stories

**US-LIST: Search and filter ISO50001BR records**
> As an **authorized user**,  
> I want to search, filter, and view ISO50001BR records in the list screen,  
...
|--------|---------|------------|
| `ISO50001BR_List` | Browse and search all ISO50001BR records | All authorized users |
| `ISO50001BR_New` | Create a new ISO50001BR request | Initiator / Requestor |
| `ISO50001BR_View` | Read-only detail view of a record | All authorized users |
| `ISO50001BR_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `ISO50001BR_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**ISO50001BR_List Screen**
- Gallery displaying all `ISO50001BR` records from `MainDB_`.
...
- `+ New` button navigates to `ISO50001BR_New` (visible to Initiator role only).
- Tap a row to navigate to `ISO50001BR_View`.

**ISO50001BR_New / _Edit Screen**
...
Set(varTitle, "ISO50001BR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ISO50001BR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ISO50001BR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(ISO50001BR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ISO50001BR` (system-enforced constant).
...
- [ ] Canvas App screens (`ISO50001BR_List`, `ISO50001BR_New`, `ISO50001BR_View`, `ISO50001BR_Edit`) are functional.
