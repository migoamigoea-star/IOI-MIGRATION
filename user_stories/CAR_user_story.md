# User Story — Cost Approval Request (`CAR`)
...
**US-01: Create & submit CAR**
> As a **Item created (FormCode=CAR)** (member of `Requester`),  
> I want to **create & submit car** in the `CAR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* `D04-FIN-Submitters`

**US-02: Cost centre review**
> As a **Status=Submitted** (member of `Cost Centre Head`),  
> I want to **cost centre review** in the `CAR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* `D04-FIN-CCHeads`

**US-03: Finance approval**
> As a **Status=Pending Approval** (member of `Finance Manager`),  
> I want to **finance approval** in the `CAR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **close workflow** in the `CAR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* `D04-FIN-Admins`

**US-LIST: Search and filter Cost Approval Request records**
> As an **authorized user**,  
> I want to search, filter, and view Cost Approval Request records in the list screen,  
...
**CAR_List Screen**
- Gallery displaying all `CAR` records from `MainDB_FIN`.
...
- `+ New` button navigates to `CAR_New` (visible to Initiator role only).
- Tap a row to navigate to `CAR_View`.

**CAR_New / _Edit Screen**
...
Set(varTitle, "CAR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(CAR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(CAR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(CAR_List, ScreenTransition.Back)
```
...
- **Stage 1:** `Create & submit CAR` — performed by `Item created (FormCode=CAR)`
- **Stage 2:** `Cost centre review` — performed by `Status=Submitted`
- **Stage 3:** `Finance approval` — performed by `Status=Pending Approval`
...
|------|----------------|
| Requester / Authors | Contribute |
| Cost Centre Head / Reviewer | Contribute |
| Finance Manager / Approver | Approve |
| System Admin | Full Control |

### 3. Data Integrity Rules

- `FormCode` must always equal `CAR` (system-enforced constant).
...
- [ ] Canvas App screens (`CAR_List`, `CAR_New`, `CAR_View`, `CAR_Edit`) are functional.
