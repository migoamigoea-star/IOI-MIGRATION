# User Story — PCN Material Disposal 2025 (`PCNMATDISPOSAL2025`)

> **Department:** STR  
> **Module:** M2 - Material Disposal  
> **Site(s):** PRAI  
> **SharePoint List:** `MainDB_STR`  
> **Form Code:** `PCNMATDISPOSAL2025`

---

## 1. App Overview & Purpose

PCNMATDISPOSAL2025 is migrated as a STR workflow form in MainDB_STR with
FormCode=PCNMATDISPOSAL2025. The implementation preserves submission, review/approval routing,
auditability, and notification behavior for PRAI operations.
...
> I want to **create and submit form** in the `PCNMATDISPOSAL2025` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=PCNMATDISPOSAL2025

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `PCNMATDISPOSAL2025` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `PCNMATDISPOSAL2025` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter PCN Material Disposal 2025 records**
> As an **authorized user**,  
> I want to search, filter, and view PCN Material Disposal 2025 records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `PCNMATDISPOSAL2025`                          |
...
|--------|---------|------------|
| PCNMATDISPOSAL2025_List | Search and filter PCNMATDISPOSAL2025 records | STR readers, reviewers, approvers |
| PCNMATDISPOSAL2025_New | Create new PCNMATDISPOSAL2025 request | STR requestors |
| PCNMATDISPOSAL2025_View | Read-only detail view | All authorized users |
| PCNMATDISPOSAL2025_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**PCNMATDISPOSAL2025_List Screen**
- Gallery displaying all `PCNMATDISPOSAL2025` records from `MainDB_STR`.
...
- `+ New` button navigates to `PCNMATDISPOSAL2025_New` (visible to Initiator role only).
- Tap a row to navigate to `PCNMATDISPOSAL2025_View`.

**PCNMATDISPOSAL2025_New / _Edit Screen**
...
**PCNMATDISPOSAL2025_View Screen**
...
Set(varTitle, "PCNMATDISPOSAL2025-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(PCNMATDISPOSAL2025_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(PCNMATDISPOSAL2025_View, ScreenTransition.None, {varRecord: ThisItem})

// Save (Patch) and navigate back to list
Patch(MainDB, {
    Title: varTitle,
    FormCode: "PCNMATDISPOSAL2025",
    Status: "Draft",
    SubmittedBy: User(),
    SubmittedDate: Now()
});
Navigate(PCNMATDISPOSAL2025_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `PCNMATDISPOSAL2025` (system-enforced constant).
...
| ------- | ------------------------------ | ------------------------------------------ | ------------------------------------------------------------------------ |
| Submit  | STR_PCNMATDISPOSAL2025_Submit  | Item created (FormCode=PCNMATDISPOSAL2025) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_PCNMATDISPOSAL2025_Review  | CurrentAction=Review                       | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_PCNMATDISPOSAL2025_Approve | Reviewer decision=Approve                  | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_PCNMATDISPOSAL2025_Close   | Final state reached                        | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`PCNMATDISPOSAL2025_List`, `PCNMATDISPOSAL2025_New`, `PCNMATDISPOSAL2025_View`, `PCNMATDISPOSAL2025_Edit`) are functional.
