# User Story — Safekeeping Direct Charge Items (`SDCI`)
...
SDCI is migrated as a STR workflow form in MainDB_STR with FormCode=SDCI. The implementation
...
> I want to **create and submit form** in the `SDCI` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SDCI

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SDCI` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SDCI` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Safekeeping Direct Charge Items records**
> As an **authorized user**,  
> I want to search, filter, and view Safekeeping Direct Charge Items records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SDCI`                                        |
...
|--------|---------|------------|
| SDCI_List | Search and filter SDCI records | STR readers, reviewers, approvers |
| SDCI_New | Create new SDCI request | STR requestors |
| SDCI_View | Read-only detail view | All authorized users |
| SDCI_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SDCI_List Screen**
- Gallery displaying all `SDCI` records from `MainDB_STR`.
...
- `+ New` button navigates to `SDCI_New` (visible to Initiator role only).
- Tap a row to navigate to `SDCI_View`.

**SDCI_New / _Edit Screen**
...
Set(varTitle, "SDCI-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SDCI_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SDCI_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SDCI_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SDCI` (system-enforced constant).
...
| ------- | ---------------- | ---------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_SDCI_Submit  | Item created (FormCode=SDCI) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SDCI_Review  | CurrentAction=Review         | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SDCI_Approve | Reviewer decision=Approve    | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SDCI_Close   | Final state reached          | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SDCI_List`, `SDCI_New`, `SDCI_View`, `SDCI_Edit`) are functional.
