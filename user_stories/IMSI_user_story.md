# User Story — INVENTORY MANAGEMENT FOR (`IMSI`)
...
IMSI is migrated as a STR workflow form in MainDB_STR with FormCode=IMSI. The implementation
...
> I want to **create and submit form** in the `IMSI` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=IMSI

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `IMSI` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `IMSI` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter INVENTORY MANAGEMENT FOR records**
> As an **authorized user**,  
> I want to search, filter, and view INVENTORY MANAGEMENT FOR records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `IMSI`                                        |
...
|--------|---------|------------|
| IMSI_List | Search and filter IMSI records | STR readers, reviewers, approvers |
| IMSI_New | Create new IMSI request | STR requestors |
| IMSI_View | Read-only detail view | All authorized users |
| IMSI_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**IMSI_List Screen**
- Gallery displaying all `IMSI` records from `MainDB_STR`.
...
- `+ New` button navigates to `IMSI_New` (visible to Initiator role only).
- Tap a row to navigate to `IMSI_View`.

**IMSI_New / _Edit Screen**
...
Set(varTitle, "IMSI-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(IMSI_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(IMSI_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(IMSI_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `IMSI` (system-enforced constant).
...
| ------- | ---------------- | ---------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_IMSI_Submit  | Item created (FormCode=IMSI) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_IMSI_Review  | CurrentAction=Review         | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_IMSI_Approve | Reviewer decision=Approve    | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_IMSI_Close   | Final state reached          | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`IMSI_List`, `IMSI_New`, `IMSI_View`, `IMSI_Edit`) are functional.
