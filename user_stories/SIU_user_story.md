# User Story — STOCK ITEM UPDATE FORM (`SIU`)
...
SIU is migrated as a STR workflow form in MainDB_STR with FormCode=SIU. The implementation preserves
...
> I want to **create and submit form** in the `SIU` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SIU

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SIU` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SIU` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter STOCK ITEM UPDATE FORM records**
> As an **authorized user**,  
> I want to search, filter, and view STOCK ITEM UPDATE FORM records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SIU`                                         |
...
|--------|---------|------------|
| SIU_List | Search and filter SIU records | STR readers, reviewers, approvers |
| SIU_New | Create new SIU request | STR requestors |
| SIU_View | Read-only detail view | All authorized users |
| SIU_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SIU_List Screen**
- Gallery displaying all `SIU` records from `MainDB_STR`.
...
- `+ New` button navigates to `SIU_New` (visible to Initiator role only).
- Tap a row to navigate to `SIU_View`.

**SIU_New / _Edit Screen**
...
Set(varTitle, "SIU-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SIU_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SIU_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SIU_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SIU` (system-enforced constant).
...
| ------- | --------------- | --------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_SIU_Submit  | Item created (FormCode=SIU) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SIU_Review  | CurrentAction=Review        | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SIU_Approve | Reviewer decision=Approve   | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SIU_Close   | Final state reached         | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SIU_List`, `SIU_New`, `SIU_View`, `SIU_Edit`) are functional.
