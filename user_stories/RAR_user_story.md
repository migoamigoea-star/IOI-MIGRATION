...
RAR is an administration form migrated from Lotus Domino to Microsoft 365 (Power Apps + SharePoint Online).
The form supports controlled administrative review and approval handling for Prai operations.
...
> I want to **submit rar** in the `RAR` application,  
...
> I want to **review rar** in the `RAR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **decide rar** in the `RAR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When CurrentStatus = 'Approved' OR 'Rejected'

**US-LIST: Search and filter RAR records**
> As an **authorized user**,  
> I want to search, filter, and view RAR records in the list screen,  
...
| 2 | FormType          | Form Type       | Single line of text    | Yes      | Fixed value: RAR                     |
...
|--------|---------|------------|
| RAR_List | List all RAR records | Authenticated users |
| RAR_New | Create RAR request | D01-ADM-Users+ |
| RAR_View | View RAR detail | Authenticated users |
| RAR_Edit | Edit RAR draft/review | Initiator + reviewers |
| RAR_Approval | Approve/reject RAR | D01-ADM-Approvers |

### Screen Interaction Details

**RAR_List Screen**
- Gallery displaying all `RAR` records from `MainDB_ADM`.
...
- `+ New` button navigates to `RAR_New` (visible to Initiator role only).
- Tap a row to navigate to `RAR_View`.

**RAR_New / _Edit Screen**
...
Set(varTitle, "RAR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(RAR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(RAR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(RAR_List, ScreenTransition.Back)
```
...
- **Stage 1:** `Submit RAR` — performed by `Initiator`
- **Stage 2:** `Review RAR` — performed by `Reviewer`
- **Stage 3:** `Decide RAR` — performed by `Approver`
...
- `FormCode` must always equal `RAR` (system-enforced constant).
...
|-------------------|---------------------------|------------------------------------------|
| ADM_RAR_OnSubmit  | CurrentStatus='Submitted' | Notify reviewer and stamp review stage   |
| ADM_RAR_OnReview  | CurrentStatus='Reviewed'  | Notify approver and stamp decision stage |
| ADM_RAR_OnApprove | CurrentStatus='Approved'  | Lock record and notify initiator         |
| ADM_RAR_OnReject  | CurrentStatus='Rejected'  | Return to initiator with reason          |
...
- [ ] Canvas App screens (`RAR_List`, `RAR_New`, `RAR_View`, `RAR_Edit`) are functional.
