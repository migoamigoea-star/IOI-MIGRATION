# User Story — Stock Return/Request Instruction 1 (`SRRI1`)
...
SRRI1 is migrated as a STR workflow form in MainDB_STR with FormCode=SRRI1. The implementation
...
> I want to **create and submit form** in the `SRRI1` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SRRI1

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SRRI1` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SRRI1` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Stock Return/Request Instruction 1 records**
> As an **authorized user**,  
> I want to search, filter, and view Stock Return/Request Instruction 1 records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SRRI1`                                       |
...
|--------|---------|------------|
| SRRI1_List | Search and filter SRRI1 records | STR readers, reviewers, approvers |
| SRRI1_New | Create new SRRI1 request | STR requestors |
| SRRI1_View | Read-only detail view | All authorized users |
| SRRI1_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SRRI1_List Screen**
- Gallery displaying all `SRRI1` records from `MainDB_STR`.
...
- `+ New` button navigates to `SRRI1_New` (visible to Initiator role only).
- Tap a row to navigate to `SRRI1_View`.

**SRRI1_New / _Edit Screen**
...
Set(varTitle, "SRRI1-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SRRI1_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SRRI1_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SRRI1_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SRRI1` (system-enforced constant).
...
| ------- | ----------------- | ----------------------------- | ------------------------------------------------------------------------ |
| Submit  | STR_SRRI1_Submit  | Item created (FormCode=SRRI1) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SRRI1_Review  | CurrentAction=Review          | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SRRI1_Approve | Reviewer decision=Approve     | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SRRI1_Close   | Final state reached           | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SRRI1_List`, `SRRI1_New`, `SRRI1_View`, `SRRI1_Edit`) are functional.
