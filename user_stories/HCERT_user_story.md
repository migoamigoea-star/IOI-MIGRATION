...
This blueprint defines the migration baseline for Hcert from Johor site source forms into
...
> I want to **draft entry** in the `HCERT` application,  
...
**US-02: Submit**
> As a **Initiator** (member of `SA-Initiators`),  
> I want to **submit** in the `HCERT` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **review** in the `HCERT` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **final decision** in the `HCERT` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* Decision action updates final status

**US-LIST: Search and filter Hcert records**
> As an **authorized user**,  
> I want to search, filter, and view Hcert records in the list screen,  
...
| 2   | FormCode       | Single line of text | Yes      | Form code           | Fixed value HCERT                            |
...
|--------|---------|------------|
| HCERT_List | Record listing and search | All authenticated roles |
| HCERT_New | New submission form | Initiators |
| HCERT_View | Read-only details | All authenticated roles |
| HCERT_Edit | Editable review form | Reviewers and approvers |

### Screen Interaction Details

**HCERT_List Screen**
- Gallery displaying all `HCERT` records from `MainDB_SA`.
...
- `+ New` button navigates to `HCERT_New` (visible to Initiator role only).
- Tap a row to navigate to `HCERT_View`.

**HCERT_New / _Edit Screen**
...
Set(varTitle, "HCERT-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(HCERT_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(HCERT_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(HCERT_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `HCERT` (system-enforced constant).
...
| ------------------------ | ------------------------ | ------------------------------------------------------------ |
| SA_HCERT_OnSubmit        | SharePoint item created  | Normalize fields, set FormCode, set CDate, notify reviewer   |
| SA_HCERT_OnReview        | SharePoint item modified | Track review decision, append audit fields, notify approver  |
| SA_HCERT_OnFinalDecision | SharePoint item modified | Apply final status and lock rules, send closure notification |
...
- [ ] Canvas App screens (`HCERT_List`, `HCERT_New`, `HCERT_View`, `HCERT_Edit`) are functional.
