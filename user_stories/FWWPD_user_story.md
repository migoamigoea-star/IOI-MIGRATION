...
FWWPD supports HR data submission and structured approval for controlled personnel workflow
processing.
...
> I want to **create draft** in the `FWWPD` application,  
...
> I want to **review** in the `FWWPD` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **final decision** in the `FWWPD` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review outcome submitted

**US-LIST: Search and filter FWWPD Form records**
> As an **authorized user**,  
> I want to search, filter, and view FWWPD Form records in the list screen,  
...
| --- | ---------------- | -------------- | --------------- | -------- | ------------------- |
| 1   | FormType         | Form Type      | Choice          | Yes      | Fixed value `FWWPD` |
...
|--------|---------|------------|
| FWWPD_List | List records | All authorized users |
| FWWPD_New | Create request | Initiator |
| FWWPD_Edit | Update request | Initiator and reviewer |
| FWWPD_View | Read-only view | All authorized users |
| FWWPD_Approval | Final decision | Manager |

### Screen Interaction Details

**FWWPD_List Screen**
- Gallery displaying all `FWWPD` records from `MainDB_HR`.
...
- `+ New` button navigates to `FWWPD_New` (visible to Initiator role only).
- Tap a row to navigate to `FWWPD_View`.

**FWWPD_New / _Edit Screen**
...
Set(varTitle, "FWWPD-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(FWWPD_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(FWWPD_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(FWWPD_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `FWWPD` (system-enforced constant).
...
| ------ | ------------------- | -------------- | -------------------------------- |
| Submit | HR_FWWPD_OnSubmit   | Item created   | Set status and notify reviewer   |
| Review | HR_FWWPD_OnReview   | Status updated | Route to manager                 |
| Final  | HR_FWWPD_OnDecision | Manager action | Lock record and notify requester |
...
- [ ] Canvas App screens (`FWWPD_List`, `FWWPD_New`, `FWWPD_View`, `FWWPD_Edit`) are functional.
