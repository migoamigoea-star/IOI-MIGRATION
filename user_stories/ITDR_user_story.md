# User Story — IT Daily Report (`ITDR`)
...
IT Daily Report is migrated as an IT workflow form in MainDB_IT with FormCode=ITDR. The
...
> I want to **create and submit form** in the `ITDR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=ITDR

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `ITDR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `ITDR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter IT Daily Report records**
> As an **authorized user**,  
> I want to search, filter, and view IT Daily Report records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `ITDR`                                        |
...
|--------|---------|------------|
| ITDR_List | Search and filter ITDR records | IT readers, reviewers, approvers |
| ITDR_New | Create new ITDR request | IT requestors |
| ITDR_View | Read-only detail view | All authorized users |
| ITDR_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**ITDR_List Screen**
- Gallery displaying all `ITDR` records from `MainDB_IT`.
...
- `+ New` button navigates to `ITDR_New` (visible to Initiator role only).
- Tap a row to navigate to `ITDR_View`.

**ITDR_New / _Edit Screen**
...
Set(varTitle, "ITDR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(ITDR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(ITDR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(ITDR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `ITDR` (system-enforced constant).
...
| ------- | --------------- | ---------------------------- | ------------------------------------------------------------------------ |
| Submit  | IT_ITDR_Submit  | Item created (FormCode=ITDR) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | IT_ITDR_Review  | CurrentAction=Review         | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | IT_ITDR_Approve | Reviewer decision=Approve    | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | IT_ITDR_Close   | Final state reached          | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`ITDR_List`, `ITDR_New`, `ITDR_View`, `ITDR_Edit`) are functional.
