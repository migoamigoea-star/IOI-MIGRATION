...
`DRD` is migrated from Lotus Domino to Microsoft 365 (Power Apps + SharePoint Online). The app supports standardized submission, review, and approval workflows for the `HR` department at the  site.

---

## 2. User Stories

**US-LIST: Search and filter DRD records**
> As an **authorized user**,  
> I want to search, filter, and view DRD records in the list screen,  
...
|--------|---------|------------|
| `DRD_List` | Browse and search all DRD records | All authorized users |
| `DRD_New` | Create a new DRD request | Initiator / Requestor |
| `DRD_View` | Read-only detail view of a record | All authorized users |
| `DRD_Edit` | Edit a draft or returned record | Initiator / Reviewer |
| `DRD_Approval` | Approve or reject the record | Approver / Manager |

### Screen Interaction Details

**DRD_List Screen**
- Gallery displaying all `DRD` records from `MainDB_HR`.
...
- `+ New` button navigates to `DRD_New` (visible to Initiator role only).
- Tap a row to navigate to `DRD_View`.

**DRD_New / _Edit Screen**
...
Set(varTitle, "DRD-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(DRD_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(DRD_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(DRD_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `DRD` (system-enforced constant).
...
- [ ] Canvas App screens (`DRD_List`, `DRD_New`, `DRD_View`, `DRD_Edit`) are functional.
