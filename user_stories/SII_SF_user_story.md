# User Story — Head of Dept's Approval (`SII_SF`)
...
SII_SF is migrated as a STR workflow form in MainDB_STR with FormCode=SII_SF. The implementation
...
> I want to **create and submit form** in the `SII_SF` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SII_SF

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D20-STR-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SII_SF` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SII_SF` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter Head of Dept's Approval records**
> As an **authorized user**,  
> I want to search, filter, and view Head of Dept's Approval records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SII_SF`                                      |
...
|--------|---------|------------|
| SII_SF_List | Search and filter SII_SF records | STR readers, reviewers, approvers |
| SII_SF_New | Create new SII_SF request | STR requestors |
| SII_SF_View | Read-only detail view | All authorized users |
| SII_SF_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SII_SF_List Screen**
- Gallery displaying all `SII_SF` records from `MainDB_STR`.
...
- `+ New` button navigates to `SII_SF_New` (visible to Initiator role only).
- Tap a row to navigate to `SII_SF_View`.

**SII_SF_New / _Edit Screen**
...
Set(varTitle, "SII_SF-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SII_SF_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SII_SF_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SII_SF_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SII_SF` (system-enforced constant).
...
| ------- | ------------------ | ------------------------------ | ------------------------------------------------------------------------ |
| Submit  | STR_SII_SF_Submit  | Item created (FormCode=SII_SF) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | STR_SII_SF_Review  | CurrentAction=Review           | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | STR_SII_SF_Approve | Reviewer decision=Approve      | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | STR_SII_SF_Close   | Final state reached            | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SII_SF_List`, `SII_SF_New`, `SII_SF_View`, `SII_SF_Edit`) are functional.
