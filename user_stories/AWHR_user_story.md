...
AWHR manages HR workflow capture and approval tracking for controlled record updates within HR
operations.
...
> I want to **create draft** in the `AWHR` application,  
...
> I want to **review** in the `AWHR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **final decision** in the `AWHR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review outcome submitted

**US-LIST: Search and filter AWHR Form records**
> As an **authorized user**,  
> I want to search, filter, and view AWHR Form records in the list screen,  
...
| --- | ---------------- | -------------- | --------------- | -------- | ------------------ |
| 1   | FormType         | Form Type      | Choice          | Yes      | Fixed value `AWHR` |
...
|--------|---------|------------|
| AWHR_List | List records | All authorized users |
| AWHR_New | Create request | Initiator |
| AWHR_Edit | Update request | Initiator and reviewer |
| AWHR_View | Read-only view | All authorized users |
| AWHR_Approval | Final decision | Manager |

### Screen Interaction Details

**AWHR_List Screen**
- Gallery displaying all `AWHR` records from `MainDB_HR`.
...
- `+ New` button navigates to `AWHR_New` (visible to Initiator role only).
- Tap a row to navigate to `AWHR_View`.

**AWHR_New / _Edit Screen**
...
Set(varTitle, "AWHR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(AWHR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(AWHR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(AWHR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `AWHR` (system-enforced constant).
...
| ------ | ------------------ | -------------- | -------------------------------- |
| Submit | HR_AWHR_OnSubmit   | Item created   | Set status and notify reviewer   |
| Review | HR_AWHR_OnReview   | Status updated | Route to manager                 |
| Final  | HR_AWHR_OnDecision | Manager action | Lock record and notify requester |
...
- [ ] Canvas App screens (`AWHR_List`, `AWHR_New`, `AWHR_View`, `AWHR_Edit`) are functional.
