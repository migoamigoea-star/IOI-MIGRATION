# User Story — SAP Authorisation Matrix Review (`SAPAMR`)
...
SAP Authorisation Matrix Review is migrated as an IT workflow form in MainDB_IT with
FormCode=SAPAMR. The implementation preserves submission, review/approval routing, auditability, and
notification behavior for PRAI operations.
...
> I want to **create and submit form** in the `SAPAMR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When item created with FormCode=SAPAMR

**US-02: Review submission and request revision if needed**
> As a **Reviewer** (member of `D06-IT-Reviewers`),  
> I want to **review submission and request revision if needed** in the `SAPAMR` application,  
> So that the workflow advances to the next approval stage.  
>
...
> I want to **approve/reject and finalize workflow** in the `SAPAMR` application,  
> So that the workflow advances to the next approval stage.  
>
> *Trigger:* When review decision recorded

**US-LIST: Search and filter SAP Authorisation Matrix Review records**
> As an **authorized user**,  
> I want to search, filter, and view SAP Authorisation Matrix Review records in the list screen,  
...
| FormCode       | Single line of text    | Yes      | Constant `SAPAMR`                                      |
...
|--------|---------|------------|
| SAPAMR_List | Search and filter SAPAMR records | IT readers, reviewers, approvers |
| SAPAMR_New | Create new SAPAMR request | IT requestors |
| SAPAMR_View | Read-only detail view | All authorized users |
| SAPAMR_Edit | Edit in draft/returned state | Requestor and reviewers |

### Screen Interaction Details

**SAPAMR_List Screen**
- Gallery displaying all `SAPAMR` records from `MainDB_IT`.
...
- `+ New` button navigates to `SAPAMR_New` (visible to Initiator role only).
- Tap a row to navigate to `SAPAMR_View`.

**SAPAMR_New / _Edit Screen**
...
Set(varTitle, "SAPAMR-" & Text(Now(), "YYYYMMDD-HHMMSS"))
```
...
Navigate(SAPAMR_New, ScreenTransition.Slide)

// Navigate from List to View screen for selected record
Navigate(SAPAMR_View, ScreenTransition.None, {varRecord: ThisItem})
...
Navigate(SAPAMR_List, ScreenTransition.Back)
```
...
- `FormCode` must always equal `SAPAMR` (system-enforced constant).
...
| ------- | ----------------- | ------------------------------ | ------------------------------------------------------------------------ |
| Submit  | IT_SAPAMR_Submit  | Item created (FormCode=SAPAMR) | Set Submitted status, stamp SubmittedBy/SubmittedDate, notify reviewers  |
| Review  | IT_SAPAMR_Review  | CurrentAction=Review           | Route for decision, persist reviewer comments, handle return/reject path |
| Approve | IT_SAPAMR_Approve | Reviewer decision=Approve      | Set Approved status, stamp ApprovedBy/ApprovedDate, notify stakeholders  |
| Close   | IT_SAPAMR_Close   | Final state reached            | Lock record and finalize notifications                                   |
...
- [ ] Canvas App screens (`SAPAMR_List`, `SAPAMR_New`, `SAPAMR_View`, `SAPAMR_Edit`) are functional.
